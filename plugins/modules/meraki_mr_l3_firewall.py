...
def main():
    ...
    if meraki.params['state'] == 'query':
        ...
        try:
            ...
            pass
        if rules['rules'][len(rules) - 2] != meraki.params['allow_lan_access']:
            update = True
        if update is True:
            ...
            if meraki.check_mode is True:
                ...
                if 'rules' in payload:
                    ...
                    cleansed_payload.append(rules['rules'][len(rules) - 1])
                    cleansed_payload.append(rules['rules'][len(rules) - 2])
                    if meraki.params['allow_lan_access'] is None:
                        cleansed_payload[len(cleansed_payload) - 2]['policy'] = rules['rules'][len(rules) - 2]['policy']
                    else:
